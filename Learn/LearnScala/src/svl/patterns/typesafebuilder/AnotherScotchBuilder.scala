...
object AnotherScotchBuilder extends App{
    ...
    builder.withBrand("Mine").withGlass(Tall).withMode(OnTheRocks).isDouble(true).build()
    builder.withBrand("Mine").withMode(OnTheRocks).withGlass(Tall).isDouble(true).build()
}
